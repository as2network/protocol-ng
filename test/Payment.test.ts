...
contract("Payment", async accounts => {
  ...
  beforeEach(async () => {
    ...
    userContract = await ArtifactUser.new(as2networkAddress);
    ...
    paymentContract = await ArtifactPayment.new(userContract.address, signatureContract.address, contractId, {
      from: as2networkAddress,
    });
  });
  ...
  it("Call init and check the correctness of parameters and if Signature has been called", async () => {
    await paymentContract.init(signatureId, documentId, startDate, endDate, period, { from: as2networkAddress });
  ...
  it("Call setReceiver and check correctness", async () => {
    await paymentContract.setReceiver(receiverId, { from: as2networkAddress });
  ...
  it("Add two receivers, call getters and check correctness", async () => {
    ...
    await paymentContract.setReceiver(secondReceiverId, { from: as2networkAddress });
  ...
  it("Call setReceiver as not as2network, expect exception", async () => {
    try {
      await paymentContract.setReceiver(receiverId, { from: invalidAddress });
  ...
  it("Call setReference without a receiver, expect receiver to be created and check correctness", async () => {
    await paymentContract.setReference(receiverId, referenceId, referenceValue, referencePrice, {
      from: as2networkAddress,
    });
  ...
  it("Set two references to the same receiver and check correctness", async () => {
    ...
    await paymentContract.setReference(receiverId, secondReferenceId, referenceValue, referencePrice, {
      from: as2networkAddress,
    });
  ...
  it("Call addPaymentCheck twice, and check correctness", async () => {
    ...
    assert.equal(
      readFirstPaymentCheckFromReference.status.toNumber(),
      readSecondPaymentCheckFromReference.status.toNumber(),
    );
    assert.equal(
      readFirstPaymentCheckFromReference.checkedAt.toNumber(),
      readSecondPaymentCheckFromReference.checkedAt.toNumber(),
    );
    assert.equal(
      readFirstPaymentCheckFromReference.createdAt.toNumber(),
      readSecondPaymentCheckFromReference.createdAt.toNumber(),
    );
