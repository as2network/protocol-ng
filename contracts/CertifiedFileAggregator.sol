...
contract CertifiedFileAggregator is NotifierInterface, BaseAggregator {
    ...
    constructor(address _userContractAddress)
        public
        BaseAggregator(_userContractAddress, "certified-file-aggregator", "certified-file-notifiers")
    {}

    function getCertifiedFileById(string memory id) public view returns (address) {
        bytes32 bytes32id = Utils.keccak(id);

        return address(certifiedFiles[bytes32id]);
    }

    function getCertifiedFile(uint256 index) public view returns (address addr, bool more) {
        ...
        if (address(certifiedFiles[certifiedFileId]) != address(0)) {
            return (address(certifiedFiles[certifiedFileId]), _more);
        }
