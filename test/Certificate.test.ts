contract("Certificate", async accounts => {
  ...
  beforeEach(async () => {
    ...
    certificateContract = await ArtifactCertificate.new(certificateId, certifiedEmailDeployer.address, {
  ...
  it("Init the contracts as CertifiedEmail account, expect to pass", async () => {
    await certificateContract.init(ownerAddress, createdAt, {
      from: certifiedEmailAddress,
    });
  ...
  it("Init the contracts as not CertifiedEmail account, expect to exception", async () => {
    try {
      ...
      assert.include(error.message, "Only CertifiedEmail account can perform this action");
    }
  });

  it("Create an instance of file", async () => {
    await certificateContract.createFile(fileHash, fileId, fileName, createdAt, fileSize, {
      from: certifiedEmailAddress,
    });
  ...
  it("Add new event to the certificate", async () => {
    await certificateContract.createEvent(eventId, eventType, userAgent, createdAt, {
      from: certifiedEmailAddress,
    });
  ...
  it("Create duplicated event", async () => {
    try {
      ...
      assert.include(error.message, "The current event already exists.");
    }
  });
