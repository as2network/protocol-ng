...
contract Clause {
    ...
    constructor(string memory notifiers) public {
        notifiersKey = notifiers;
    }

    function _notifySignature(string memory creationEvent) internal {
        signatureContract.notify(creationEvent, address(this));
    }

    function _notify(string memory eventType) internal {
        ...
        do {
            ...
            if (contractToNofify != address(0)) {
                contractToNofify.call(abi.encodeWithSignature("notify(string,address)", eventType, address(this)));
            }
