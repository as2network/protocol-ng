...
var AS2networkUser = artifacts.require("./AS2networkUser.sol");
...
module.exports = async function (deployer, network, accounts) {
  ...
  deployer.then(async () => {
    const signatureDeployerInstance = await deployer.deploy(SignatureDeployer);
    ...
    const as2networkUser = await deployer.deploy(AS2networkUser, accounts[0]);

    tx = await web3.eth.getTransactionReceipt(as2networkUser.transactionHash);
    ...
    const certifiedEmailDeployerInstance = await deployer.deploy(CertifiedEmailDeployer);
    ...
    const documentInstance = await deployer.deploy(Document, v4(), signatureDeployerInstance.address);
    ...
    const certificateInstance = await deployer.deploy(Certificate, v4(), certifiedEmailDeployerInstance.address);
    ...
    const eventInstance = await deployer.deploy(Event, "event id", "event type", "event user agent", 1500);
    ...
    const certifiedFileChecker = await deployer.deploy(CertifiedFileChecker);
    ...
    const certifiedFileAggregator = await deployer.deploy(CertifiedFileAggregator, as2networkUser.address);
    ...
    const signatureAggregator = await deployer.deploy(SignatureAggregator, as2networkUser.address);
    ...
    const certifiedEmailAggregator = await deployer.deploy(CertifiedEmailAggregator, as2networkUser.address);
    ...
    const userEvents = await deployer.deploy(UserEvents, as2networkUser.address);
    ...
    const userAuthority = await deployer.deploy(UserAuthority);
