contract("SignatureAggregator", async accounts => {
  ...
  it("Deploy signature aggregator contract from invalid account", async () => {
    try {
      await ArtifactSignatureAggregator.new(userContract.address, {
        from: invalidAccount,
      });
    } catch (error) {
  ...
  it("Create signature without address to notify and notify manually", async () => {
    ...
    await signatureAggregatorContract.notify("signature.contract.created", signatureContract.address);
