contract("CertifiedFileAggregator", async accounts => {
  ...
  beforeEach(async () => {
    ...
    certifiedFileAggregatorContract = await ArtifactCertifiedFileAggregator.new(userContract.address, {
      from: rootAddress,
    });
  });
  ...
  it("Deploy certified file contract from invalid account, expect exception", async () => {
    try {
      await ArtifactCertifiedFileAggregator.new(userContract.address, {
        from: invalidAccount,
      });
    } catch (error) {
  ...
  it("Create certified file without address to notify and notify manually", async () => {
    ...
    await certifiedFileAggregatorContract.notify("certified_file.contract.created", certifiedFileContract.address);
  ...
  it("Add the address of the notify checker and expect to be notified", async () => {
    ...
    await userContract.setAddressArrayAttribute("certified-file-notifiers", certifiedFileChecker.address);
    ...
    const certifiedFile = await certifiedFileChecker.getFile(certifiedFileHash, 0);
