contract("File", async accounts => {
  ...
  let as2networkAddress = accounts[0];
  ...
  beforeEach(async () => {
    fileContract = await ArtifactFile.new(fileId, {
  ...
  it("Try to initialize with invalid fileSize, expect exception", async () => {
    try {
      const initTransaction = await fileContract.init(fileName, fileHash, fileCreatedAt, badFileSize, {
        from: as2networkAddress,
      });

      assert.fail("It should have thrown");
    } catch (error) {
      assert.include(error.message, "Invalid input value(s).");
    }
  });

  it("Try to initialize as parent role, expect to pass and check correctness", async () => {
    const initTransaction = await fileContract.init(fileName, fileHash, fileCreatedAt, fileSize);
  ...
  it("Try to initialize as not parent role, expect exception", async () => {
    try {
      ...
      assert.include(error.message, "Only the parent account can perform this action.");
    }
  });
});
