contract("UserAuthority", async accounts => {
  ...
  it("create user from user account, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.createUser(secondUserAddress, {
        from: userAddress,
      });
    } catch (error) {
      assert.include(error.message, "The account can't perform this action");
  ...
  it("Set user contract from manager, expect to pass", async () => {
    ...
    await userAuthorityContract.setUserContract(userAddress, secondUserAddress, {
      from: adminAddress,
    });
  ...
  it("Set user contract from user, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.setUserContract(userAddress, adminAddress, {
  ...
  it("Set user role as admin from manager, expect to pass", async () => {
    ...
    await userAuthorityContract.setRole(userAddress, UserRole.admin, {
      from: adminAddress,
    });
  ...
  it("Set user role as admin from user, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.setRole(userAddress, UserRole.user, {
        from: userAddress,
      });
    } catch (error) {
      assert.include(error.message, "Unauthorized action");
  ...
  it("Set user role as root from manager, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.setRole(userAddress, UserRole.root, {
        from: adminAddress,
      });
    } catch (error) {
  ...
  it("Set user reputation as admin from manager, expect to pass", async () => {
    ...
    await userAuthorityContract.setReputation(userAddress, 1, {
      from: adminAddress,
    });
  ...
  it("Set user reputation as admin from user, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.setReputation(secondUserAddress, 1, {
        from: userAddress,
      });
    } catch (error) {
      assert.include(error.message, "Unauthorized action");
  ...
  it("Set user validity as admin from manager, expect to pass", async () => {
    ...
    await userAuthorityContract.setValidity(userAddress, false, {
      from: adminAddress,
    });
  ...
  it("Set user validity as admin from user, expect exception", async () => {
    ...
    try {
      await userAuthorityContract.setValidity(secondUserAddress, false, {
        from: userAddress,
      });
    } catch (error) {
      assert.include(error.message, "Unauthorized action");
