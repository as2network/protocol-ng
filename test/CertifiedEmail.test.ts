contract("Certified Email", async accounts => {
  ...
  beforeEach(async () => {
    ...
    userContract = await ArtifactUser.new(accounts[4], {
      from: as2networkAddress,
    });

    certifiedEmailAggregatorContract = await ArtifactCertifiedEmailAggregator.new(userContract.address, {
      from: as2networkAddress,
    });
  ...
  it("Check the owner address correctness", async () => {
    ...
    assert.equal(as2networkAddress, readOwnerContractAddress);
  });
  ...
  it("Create certificate on the certified email and check stored parameters correctness", async () => {
    await certifiedEmailContract.createCertificate(certificateId, createdAt, ownerAddress, {
      from: as2networkAddress,
    });
  ...
  it("Create certificate on the certified email from not AS2network account, expect exception", async () => {
    try {
      await certifiedEmailContract.createCertificate(certificateId, createdAt, ownerAddress, {
        from: invalidAddress,
      });
  ...
  it("Retrieve uncreated certificate from certificateId", async () => {
    try {
      ...
      assert.include(error.message, "The certificate with this id doesn't exist");
    }
  });
  ...
  it("Create event on created certificate and check parameters correctness", async () => {
    ...
    await certifiedEmailContract.createEvent(certificateId, eventId, eventType, eventUserAgent, createdAt);
  ...
  it("notify creation from as2network account", async () => {
    ...
    assert.equal(as2networkAddress, readOwnerAddress);
  ...
  it("Create event as not AS2network account, expect exception", async () => {
    try {
      await certifiedEmailContract.createEvent(certificateId, eventId, eventType, eventUserAgent, createdAt, {
        from: invalidAddress,
      });
