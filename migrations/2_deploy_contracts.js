...
module.exports = async function (deployer, network, accounts) {
	...
	deployer.then(async () => {
		...
		var tx = await web3.eth.getTransactionReceipt(
			signatureDeployerInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			signatureInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedEmailDeployerInstance.transactionHash
		);
		console.log(
			"GAS USED FOR CERTIFIED EMAIL DEPLOYER: " + tx.cumulativeGasUsed
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedEmailInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certificateInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			timeLoggerInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedFileInstance.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedFileChecker.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedFileAggregator.transactionHash
		);
		console.log(
			"GAS USED FOR CERTIFIED FILE AGGREGATOR: " + tx.cumulativeGasUsed
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			signatureAggregator.transactionHash
		);
		...
		tx = await web3.eth.getTransactionReceipt(
			certifiedEmailAggregator.transactionHash
		);
		console.log(
			"GAS USED FOR CERTIFIED EMAIL AGGREGATOR: " + tx.cumulativeGasUsed
		);
