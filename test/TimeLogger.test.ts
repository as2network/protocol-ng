contract("TimeLogger", async accounts => {
  ...
  it("Call init as AS2network account, expect to pass", async () => {
  ...
  it("Call externalSourceLog as Owner account, expect to pass", async () => {
    await timeLoggerContract.externalSourceLog(timeNow, { from: managerAddress });
  ...
  it("Call externalSourceLog as not Owner account, expect exception", async () => {
    try {
      await timeLoggerContract.externalSourceLog(timeNow, { from: ownerAddress });

      assert.fail("Something went wrong, it should have thrown");
    } catch (error) {
      assert.include(error.message, "Only the manager account can perform this action.");
    }
  });

  it("Call externalSourceLog when expired, expect exception", async () => {
    await timeLoggerContract.expireContract({ from: managerAddress });
    ...
    try {
      ...
      assert.include(error.message, "This contract has expired");
    }
  });

  it("Call soliditySourceLog as Owner account, expect to pass", async () => {
    await timeLoggerContract.soliditySourceLog({ from: ownerAddress });
  ...
  it("Call soliditySourceLog and then externalSourceLog, expect final source to be external", async () => {
    ...
    await timeLoggerContract.externalSourceLog(timeNow + 1000, { from: managerAddress });
  ...
  it("Call createTimeLog as manager account, expect to pass", async () => {
    ...
    await timeLoggerContract.createTimeLog(daysSinceUnixEpoch, timeNow, timeNow + 100, { from: managerAddress });
  ...
  it("Call createTimeLog as not manager account, expect exception", async () => {
    ...
    try {
      await timeLoggerContract.createTimeLog(daysSinceUnixEpoch, timeNow, timeNow + 100, { from: invalidAddress });
  ...
  it("Call createTimeLog with a close date before the opening one, expect exception", async () => {
    ...
    try {
      await timeLoggerContract.createTimeLog(daysSinceUnixEpoch, timeNow + 100, timeNow, { from: managerAddress });

      assert.fail("Something went wrong, it should have thrown");
    } catch (error) {
      assert.include(error.message, "Invalid time frame");
    }
  });
  ...
  it("Call editTimeLog as manager account, expect to pass", async () => {
    ...
    await timeLoggerContract.editTimeLog(daysSinceUnixEpoch, 0, timeNow, timeNow + 50, true, { from: managerAddress });
  ...
  it("Call editTimeLog as not manager account, expect exception", async () => {
    ...
    try {
      await timeLoggerContract.editTimeLog(daysSinceUnixEpoch, 0, timeNow, timeNow + 50, true, {
        from: invalidAddress,
      });
  ...
  it("Call editTimeLog with invalid dates (end > start), expect exception", async () => {
    ...
    try {
      await timeLoggerContract.editTimeLog(daysSinceUnixEpoch, 0, timeNow + 50, timeNow, true, {
        from: managerAddress,
      });
  ...
  it("Call setExpiration as not manager account, expect exception", async () => {
    try {
      await timeLoggerContract.expireContract({ from: invalidAddress });
  ...
  it("Log time three times into two different days", async () => {
    await timeLoggerContract.externalSourceLog(timeTwoDaysAgo, { from: managerAddress });

    await timeLoggerContract.externalSourceLog(timeTwoDaysAgo + 100, { from: managerAddress });
    ...
    try {
      ...
      assert.include(error.message, "There is no log for this day");
    }
  ...
  it("Log time twice, expect a timelog to be opened and closed and the total of the day updated", async () => {
    ...
    await timeLoggerContract.externalSourceLog(timeNow + 100, { from: managerAddress });
  ...
  it("Leave a day open and log time two days after, expect gap to be filled and old day closed", async () => {
    ...
    assert.ok(readThirdTimeLog.end.toNumber() > 0);
    assert.ok(
      readSecondTimeLog.end.toNumber() == todayMidnight && readSecondTimeLog.start.toNumber() == startOfSecondDay,
    );
  });
  ...
  it("Call getTotalLoggedTime after having logged, expect to receive the total", async () => {
    ...
    await timeLoggerContract.externalSourceLog(timeNow + 50, { from: managerAddress });

    await timeLoggerContract.externalSourceLog(timeNow + secondsInDay, { from: managerAddress });

    await timeLoggerContract.externalSourceLog(timeNow + secondsInDay + 50, { from: managerAddress });
  ...
  it("Call getTotalLoggedTime of an extremely big timeframe, expect to run out of gas", async () => {
    ...
    try {
      ...
      assert.include(error.message, "out of gas");
    }
  });
