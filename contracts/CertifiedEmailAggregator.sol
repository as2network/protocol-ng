...
contract CertifiedEmailAggregator is NotifierInterface, BaseAggregator {
    ...
    constructor(address _userContractAddress)
        public
        BaseAggregator(_userContractAddress, "certified-email-aggregator", "certified-email-notifiers")
    {}

    function getCertifiedEmailById(string memory id) public view returns (address) {
        bytes32 bytes32id = Utils.keccak(id);

        return address(certifiedEmails[bytes32id]);
    }

    function getCertifiedEmail(uint256 index) public view returns (address addr, bool more) {
        ...
        if (address(certifiedEmails[certifiedEmailId]) != address(0)) {
            return (address(certifiedEmails[certifiedEmailId]), _more);
        }
