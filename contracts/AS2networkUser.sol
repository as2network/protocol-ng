...
contract AS2networkUser is AS2networkUserInterface {
    ...
    modifier protected() {
        require(tx.origin == rootAddress, "Only the owner can perform this action");

        _;
    }

    function setStringAttribute(string memory key, string memory value) public protected {
        bytes32 bytes32key = Utils.keccak(key);

        stringAttr[bytes32key] = value;
    }

    function deleteStringAttribute(string memory key) public protected {
        bytes32 bytes32key = Utils.keccak(key);
    ...
    function setStringArrayAttribute(string memory key, string memory value) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        stringArrayAttr[bytes32key].push(value);
    }

    function deleteStringArrayAttribute(string memory key, uint256 index) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        uint256 arrayLength = stringArrayAttr[bytes32key].length;

        if (index >= arrayLength) return;

        for (uint256 i = index; i < arrayLength - 1; i++) {
    ...
    function setNumberAttribute(string memory key, int256 value) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        numberAttr[bytes32key] = value;
    }

    function deleteNumberAttribute(string memory key) public protected {
        bytes32 bytes32key = Utils.keccak(key);
    ...
    function setNumberArrayAttribute(string memory key, int256 value) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        numberArrayAttr[bytes32key].push(value);
    }

    function deleteNumberArrayAttribute(string memory key, uint256 index) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        uint256 arrayLength = numberArrayAttr[bytes32key].length;
    ...
    function setAddressAttribute(string memory key, address value) public protected {
        bytes32 bytes32key = Utils.keccak(key);

        addressAttr[bytes32key] = value;
    }

    function deleteAddressAttribute(string memory key) public protected {
        bytes32 bytes32key = Utils.keccak(key);
    ...
    function setAddressArrayAttribute(string memory key, address value) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        addressArrayAttr[bytes32key].push(value);
    }

    function deleteAddressArrayAttribute(string memory key, uint256 index) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        uint256 arrayLength = addressArrayAttr[bytes32key].length;
    ...
    function setBooleanAttribute(string memory key, bool value) public protected {
        bytes32 bytes32key = Utils.keccak(key);

        boolAttr[bytes32key] = value;
    }

    function deleteBooleanAttribute(string memory key) public protected {
        bytes32 bytes32key = Utils.keccak(key);
    ...
    function setBooleanArrayAttribute(string memory key, bool value) public protected {
        bytes32 bytes32key = Utils.keccak(key);
    ...
    function deleteBooleanArrayAttribute(string memory key, uint256 index) public protected {
        bytes32 bytes32key = Utils.keccak(key);
        uint256 arrayLength = boolArrayAttr[bytes32key].length;
