contract("CertifiedFileChecker", async accounts => {
  ...
  it("Add file to the certified file checker", async () => {
    await certifiedFileChecker.notify("certified_file.contract.created", fileContract.address, {
      from: as2networkAddress,
    });
  });

  it("Try to add a certified file from other account than as2network", async () => {
    try {
      await certifiedFileChecker.notify("certified_file.contract.created", fileContract.address, {
        from: ownerAddress,
      });
  ...
  it("Access to the second certified file", async () => {
    ...
    await certifiedFileChecker.notify("certified_file.contract.created", fileContract2.address, {
      from: as2networkAddress,
    });
